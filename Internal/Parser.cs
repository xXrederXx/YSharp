...
public class Parser
{
    ...
    private ParseResult CompExpr()
    {
        ...
        while (currentToken.IsType(TokenType.EE, TokenType.NE, TokenType.GT, TokenType.LT, TokenType.GTE, TokenType.LTE))
        {
            Token<TokenNoValueType> opTok = (Token<TokenNoValueType>)currentToken;
    ...
    private ParseResult Expression()
    {
        ...
        while (
            currentToken.Matches(TokenType.KEYWORD, "AND")
            || currentToken.Matches(TokenType.KEYWORD, "OR")
        )
        {
            IToken opTok = currentToken;
    ...
    private ParseResult Statement()
    {
        ...
        if (res.HasError)
        {
            return res.Failure(
                new InvalidSyntaxError(
                    currentToken.StartPos,
                    "expected 'RETURN', 'CONTINUE', 'BREAK', 'VAR', 'IF', 'WHILE', 'FUN', int, float, identifier, '+', '-', '(', '['"
                )
            );
        }
