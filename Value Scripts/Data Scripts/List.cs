...
public class VList(List<Value> elements) : Value
{
    ...
    { // converts a value to a csharp usable index
        ...
        {
            return (0, new ArgOutOfRangeError(argValues[0].startPos, "Index was out of range. Must be less than size of list.", context));
        }
        ...
        if (index < 0)
        {
            return (0, new ArgOutOfRangeError(argValues[0].startPos, "Index was out of range. Negative size cant be greater than size of list.", context));
        }
    ...
    private VNumber IndexOf(Value value)
    {
        ...
        int index = value switch
        {
            ...
            VList list => elements.FindIndex(v => v is VList l && l.elements.SequenceEqual(list.elements)),
            _ => -1
        };
