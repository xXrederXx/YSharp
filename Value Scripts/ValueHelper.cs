...
public static class ValueHelper{
    public static Error IsRightLength(int length, List<Value> argValue, Context? context)
    {
        ...
        return new NumArgsError(Position.Null, $"Should have {length} args not {argValue.Count}", context);
    }
    public static Error IsRightType(List<Type> types, List<Value> argValue, Context context)
    {
        ...
        for (int i = 0; i < argValue.Count; i++)
        {
            ...
            { // one type
                return new WrongFormatError(Position.Null, $"Value should have Type {types[0]} and not {valType}", context);
            }
            else if (valType != types[i])
            {
                return new WrongFormatError(Position.Null, $"Value should have {types[0]} and not Type {valType}", context);
            }
        }
        return ErrorNull.Instance;
    }
    public static Error CheckArgs(List<Value> argValue, int length, List<Type> types, Context? context)
    {
        Error ret;
